...
const DM_SESSIONS_DIR = path.join(process.cwd(), 'logs', 'sessions', 'dm');
...
function loadDMs(): TrackedDM[] {
    ensureDirs();
    return safeReadJSON<TrackedDM[]>(DM_FILE, [], 'dm_messages');
}

function saveDMs(dms: TrackedDM[]) {
    ensureDirs();
    if (!safeWriteJSON(DM_FILE, dms, 'dm_messages')) {
        logger.error('[dm-tracker] DM data may be lost — write failed');
    }
}

// ── Public API ───────────────────────────────────────────────────────

export function hasSentDMTo(username: string, withinHours = 24): TrackedDM | null {
...
export function trackDM(dm: TrackedDM) {
    ...
    dms.push(dm);
    saveDMs(dms);
    logger.info(`[dm-tracker] Recorded ${dm.direction} DM to @${dm.recipientUsername}`);
}
...
export function getDMsForUser(username: string): TrackedDM[] {
    const dms = loadDMs();
...
export function cleanupOldDMs(daysToKeep = 90) {
    ...
    if (filtered.length < dms.length) {
        saveDMs(filtered);
        logger.info(`[dm-tracker] Cleaned up ${dms.length - filtered.length} old DMs`);
    }
}

// ── Session logging ──────────────────────────────────────────────────

export function createDMSession(): DMSessionLog {
    const sessionId = `dm_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    return {
        sessionId,
...
export function saveDMSession(session: DMSessionLog) {
    try {
        ensureDirs();
        ...
        logger.info(`[dm-tracker] Session saved: ${session.sessionId}`);
    } catch (e) {
        logger.error('[dm-tracker] Failed to save session', e);
    }
}
