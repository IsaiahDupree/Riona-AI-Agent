...
const RELATIONSHIPS_DIR = path.join(process.cwd(), 'logs', 'tracking', 'dm', 'relationships');
...
export function loadRelationship(username: string): RelationshipInfo {
    ...
    } catch (e) { logger.warn('[dm-ai] Failed to load relationship: ' + formatError(e)); }
    return {
        category: 'personal',
        warmth: 0,
        stage: 'cold_outreach',
...
export function saveRelationship(username: string, rel: RelationshipInfo) {
    try {
        ...
        logger.info(`[dm-ai] Saved relationship for @${username}: ${rel.category}, warmth=${rel.warmth}, stage=${rel.stage}`);

        // Sync to Supabase (fire-and-forget)
        syncRelationshipToSupabase(username, rel.category, rel.warmth, rel.stage, rel.tags || []).catch(() => { });
    } catch (e) {
        logger.error('[dm-ai] Failed to save relationship', e);
    }
}
...
export function getAllRelationships(): Record<string, RelationshipInfo> {
    try {
        ...
        return rels;
    } catch (e) { logger.warn('[dm-ai] Failed to load all relationships: ' + formatError(e)); }
    return {};
}
...
2. Reference something specific about their profile or content when possible
    ...
    switch (rel.stage) {
        case 'cold_outreach':
            if (rel.category === 'business_networking')
                return 'Start a conversation by finding common ground in business/tech. Be curious about their work.';
            if (rel.category === 'collaborator')
                return 'Compliment their content genuinely and start a casual conversation.';
            if (rel.category === 'potential_client')
                return 'Start with genuine interest in their business. Ask about what they do.';
            return 'Start a friendly, casual conversation. Find something in common.';
...
export function recordFeedback(feedback: MessageFeedback) {
    try {
        ...
        } catch (e) { logger.warn('[dm-ai] Failed to record timing stat: ' + formatError(e)); }

        // Sync to Supabase (fire-and-forget)
        syncFeedbackToSupabase(feedback).catch(() => { });

        logger.info(`[dm-ai] Feedback recorded for @${feedback.recipientUsername}: reply=${feedback.gotReply}, sentiment=${feedback.replySentiment || 'n/a'}`);
    } catch (e) {
        logger.error('[dm-ai] Failed to record feedback', e);
    }
}
...
export function getFeedbackStats(): { totalSent: number; gotReply: number; replyRate: number; sentimentBreakdown: Record<string, number> } {
    ...
    } catch (e) { logger.warn('[dm-ai] Failed to load feedback stats: ' + formatError(e)); }
