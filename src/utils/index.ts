...
export function checkRequiredEnvVars(): boolean {
    ...
    const requiredVars = [
        ...
        'INSTAGRAM_PROXY_PORT',
        'THREADS_BOT_USERNAME',
        'THREADS_BOT_PASSWORD',
        'THREADS_PROXY_PORT',
        'MONGODB_URI'
    ];
...
export async function Instagram_cookiesExist(cookiesPath: string): Promise<boolean> {
    try {
        ...
        if (err.code === 'ENOENT') {
            logger.warn(`Cookies file does not exist at ${cookiesPath}`);
            return false;
        } else {
...
export async function saveCookies(cookiesPath: string, cookies: any[]): Promise<void> {
    try {
        ...
        logger.info(`Cookies saved successfully to ${cookiesPath}`);
    } catch (error) {
        logger.error(`Error saving cookies to ${cookiesPath}:`, error);
    }
}
...
export async function loadCookies(cookiesPath: string): Promise<any[]> {
    try {
        ...
        logger.error(`Error loading cookies from ${cookiesPath}:`, error);
        return [];
    }
}
